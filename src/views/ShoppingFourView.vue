...
export default {
  ...
  methods: {
    ...
    getStorage() {
      try {
        const shopList = JSON.parse(sessionStorage.getItem('shopping-list')) || [];
        const pracelData = JSON.parse(sessionStorage.getItem('pracel-data')) || {};
        const count = JSON.parse(sessionStorage.getItem('total-count')) || '';
        const price = JSON.parse(sessionStorage.getItem('total-price')) || '';
        const pay = JSON.parse(sessionStorage.getItem('choose-Pay')) || '';
        const parcel = JSON.parse(sessionStorage.getItem('choose-parcel')) || '';
        this.shopList = shopList;
        this.pracel = pracelData;
        this.totalPrice = price;
        this.totalCount = count;
        this.choosePay = pay;
        this.chooseParcel = parcel;
      } catch (error) {
        console.error(error);
      }
    },
...
<template>
  <main class="py-5">
    <div class="container px-4 px-lg-5 bg-success bg-opacity-25">
      ...
      <h6>姓名：
        <span>{{ pracel.name }}</span>
      </h6>
      <h6>電話：
        <span>{{ pracel.phone }}</span>
      </h6>
      <h6>Email：
        <span>{{ pracel.email }}</span>
      </h6>
      <h6>付款方式：
        <span>{{ choosePay }}</span>
      </h6>
      <h6>寄送方式：
        <span>{{ chooseParcel }}</span>
      </h6>
      <h6 v-if="chooseParcel === '黑貓宅配'">地址：
        <span>{{ pracel.city }}{{ pracel.postcode }}{{ pracel.address }}</span>
      </h6>
      <br>
      ...
      <section class="row">
        ...
        <ul class="nav px-3 py-5 ">
          <li class="nav-item">
            <RouterLink class="nav-link" to="/Receiver">
              ⬅️ 上一步
            </RouterLink>
          </li>
          <li class="nav-item ms-auto">
            <RouterLink class="btn btn-success px-5" to="/">完成訂購</RouterLink>
          </li>
        </ul>
      </section>
    </div>
