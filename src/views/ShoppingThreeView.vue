...
export default {
  ...
  methods: {
    setStorage() {
      sessionStorage.setItem('pracel-data', JSON.stringify(this.parcelData));
    },
    getStorage() {
      try {
    ...
    <div class="container px-4 px-lg-5 bg-success bg-opacity-25">
      <section class="row">
        ...
        <div class="row mx-auto">
          ...
          <div class="col position-relative">
            <div class="position-absolute top-50 start-50 translate-middle w-100">
              <div class="progress" style="height: 10px">
                <div class="progress-bar w-75 progress-bar-striped progress-bar-animated bg-success" role="progressbar">
                </div>
              </div>
              <div class="fs-7 fw-light pt-1"><br></div>
            </div>
      ...
      <section class="row pt-3 px-5">
        ...
        <form class="">
          <div class="mb-3">
            ...
            <input v-model="parcelData.name" type="text" class="form-control" id="exampleFormControlInput1"
              placeholder="請輸入姓名">
          </div>
          <div class="mb-3">
            <label for="exampleFormControlInput1" class="form-label">電話：</label>
            <input v-model="parcelData.phone" type="text" class="form-control" id="exampleFormControlInput2"
              placeholder="請輸入電話">
          </div>
          <div class="mb-3">
            <label for="exampleFormControlInput1" class="form-label">Email：</label>
            <input v-model="parcelData.email" type="email" class="form-control" id="exampleFormControlInput3"
              placeholder="請輸入Email">
          </div>
          <label for="exampleFormControlInput1" class="form-label">地址：</label>
          <div class="mb-3 d-flex">
            <input v-model="parcelData.city" type="text" class="form-control" placeholder="請輸入城市">
            <span>&nbsp;</span>
            <input v-model="parcelData.postcode" type="text" class="form-control" placeholder="請輸入郵遞區號">
          </div>
          <input v-model="parcelData.address" type="text" class="form-control" placeholder="請輸入完整地址">
        </form>
      </section>
      <br>
      <div class="border-top border-success border-2 pt-3"></div>
      <section class="row">
      ...
      <section>
        <ul class="nav px-3 py-5 ">
          <li class="nav-item">
            <RouterLink class="nav-link" to="/ShoppingTwo">⬅️ 上一步</RouterLink>
          </li>
          <li class="nav-item ms-auto">
            <RouterLink class="btn btn-success px-5" to="/ShoppingFour" @click="setStorage">下一步</RouterLink>
          </li>
        </ul>
      </section>
    </div>
